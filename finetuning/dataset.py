...
class TTSDataset(Dataset):
    ...
    def _normalize_audio_inputs(
        ...
        for a in items:
            if a is None:
                raise ValueError(
                    "Audio path is None. Re-run prepare_data.py to regenerate "
                    "the JSONL with valid ref_audio paths."
                )
            if isinstance(a, str):
    ...
    def extract_mels(self, audio: np.ndarray, sr: int) -> torch.Tensor:
        assert sr == TARGET_SR, (
            f"extract_mels expects {TARGET_SR} Hz audio, got {sr} Hz."
        )
    ...
    def __getitem__(self, idx):
        ...
        ref_audio_path = item.get("ref_audio") or audio_path

        if ref_audio_path is None:
            raise ValueError(
                f"Sample {idx}: both 'ref_audio' and 'audio' are None. "
                "Please re-run prepare_data.py with the fixed version."
            )
    ...
    def collate_fn(self, batch):
        ...
        # FIX: ref_mel has shape (1, T_i, 128) where T_i differs per sample
        # because audio clips have different durations. Pad all to max_mel_len
        # along the time dimension before concatenating into a single tensor.
        mel_list = [data["ref_mel"] for data in batch]   # each: (1, T_i, 128)
        max_mel_len = max(m.shape[1] for m in mel_list)
        padded_mels = []
        for m in mel_list:
            pad_len = max_mel_len - m.shape[1]
            if pad_len > 0:
                # F.pad pads last dim first, so to pad dim=1 (time):
                # pad = (last_dim_left, last_dim_right, dim1_left, dim1_right)
                m = F.pad(m, (0, 0, 0, pad_len))
            padded_mels.append(m)
        ref_mels = torch.cat(padded_mels, dim=0)         # (B, max_mel_len, 128)
